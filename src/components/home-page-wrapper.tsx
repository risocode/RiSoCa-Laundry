  ...
  const mainContent = React.Children.map(children, (child) => {
    ...
    const replaceGrid = (node: React.ReactNode): React.ReactNode => {
      ...
      if (typeof className === 'string' && className.includes('grid')) {
         return (
            <div key="grid-wrapper" className="w-full max-w-sm sm:max-w-md overflow-x-auto pb-4">
                ...
                {gridItems.map((item) => {
                  ...
                  return (
                    <Wrapper
                      key={item.label}
                      {...props}
                      className={cn("relative flex flex-col items-center justify-center gap-1 p-1 rounded-lg group min-w-[80px]", isComingSoon ? 'opacity-50 cursor-not-allowed' : '')}
                    >
