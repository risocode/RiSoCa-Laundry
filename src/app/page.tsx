    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 flex flex-col items-center justify-center container mx-auto px-4 py-8 text-center">
        ...
        <div className="flex items-center justify-center space-x-4 mb-8">
           ...
           <div>
            <h1 className="text-3xl font-bold tracking-tight text-primary text-left">RKR Laundry</h1>
            <p className="text-md text-muted-foreground text-left">Fast. Clean. Convenient.</p>
           </div>
        </div>

        <div className="flex flex-row items-center gap-4 mb-12">
          <Link href="/login" passHref>
            <Button size="lg" className="w-36 h-12 text-base rounded-full bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-lg hover:shadow-xl transition-shadow">
              <LogIn className="mr-2 h-4 w-4" />
              Log In
            </Button>
          </Link>
          <Link href="/register" passHref>
            <Button size="lg" className="w-36 h-12 text-base rounded-full bg-gradient-to-r from-amber-400 to-yellow-500 text-white shadow-lg hover:shadow-xl transition-shadow">
        ...
        <div className="grid grid-cols-3 gap-x-8 gap-y-6 w-full max-w-lg">
          {gridItems.map((item) => (
            <Link href={item.href} key={item.label} className="relative flex flex-col items-center justify-center gap-2 p-2 rounded-lg group">
              <item.icon className="h-8 w-8 text-foreground/80 group-hover:text-primary transition-colors" />
